...
df = pd.DataFrame(columns=['title','thumb','epi_title','id'])
browser = mechanicalsoup.StatefulBrowser()
driver = webdriver.Chrome('chromedriver.exe')

list = ['daily', 'comic', 'fantasy', 'action', 'drama', 'pure', 'sensibility', 'thrill', 'historical', 'sports']

df = pd.DataFrame(columns=['title','episode_title','img_url', 'id'])
        ...
        while url_soup.select('a.next'):
            for tag2 in url_soup.select('tr'):
                if tag2.find('img') == None:
                    ...
                    results = [(title,epi_title,thumb,wid)]
                    dfNew = pd.DataFrame(results, columns=['title','episode_title','img_url','id'])
